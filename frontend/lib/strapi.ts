...
const R2_PRIVATE = "https://ae0e5ad00759ea5f03237821fe4e6ca9.r2.cloudflarestorage.com/fwip-sk-media";
...
export async function fetchHeroVideo(): Promise<HeroVideo | null> {
  const res = await fetch(
    `${STRAPI}/api/hero-videos?populate=*`,
    { next: { revalidate: 60 } }
  );
  ...
  return {
    ...
    video: { url: toAbsoluteUrl(hero.video.url) },
  };
}

export async function fetchHeroVideoIceCream(): Promise<HeroVideo | null> {
  const res = await fetch(
    `${STRAPI}/api/hero-video-ice-creams?populate=*`,
    { next: { revalidate: 60 } }
  );
  ...
  return {
    ...
    textField2: hero.textField2 ?? undefined,
    textField3: hero.textField3 ?? undefined,
    video: { url: toAbsoluteUrl(hero.video[0].url) },
  };
}
...
export async function fetchIceCreams(): Promise<IceCream[]> {
  try {
    const res = await fetch(
      `${STRAPI}/api/ice-creams?populate=*`,
      { next: { revalidate: 60 } }
    );
    ...
    return data.data.map((item: any) => {
      ...
      const rawUrl = img?.formats?.large?.url || img?.formats?.medium?.url || img?.url || "";
      const fullImageUrl = toAbsoluteUrl(rawUrl);
...
export async function fetchPortobelloData(): Promise<PortobelloData> {
  try {
    ...
    return {
      ...
      image: item?.image
        ? {
            url: toAbsoluteUrl(item.image.url),
  ...
  const features =
    (secondarySection?.imageTextCombo || [])
      ...
      .map((item: ImageTextComboItem) => ({
        iconUrl: item.image?.url ? toAbsoluteUrl(item.image.url) : undefined,
...
export async function fetchPromoSections(): Promise<PromoSectionData[]> {
  try {
    const res = await fetch(
      `${STRAPI}/api/promo-sections?populate=image`,
      { next: { revalidate: 60 } }
    );
...
export async function fetchHeroVideoToSeparate(): Promise<HeroVideoToSeparate | null> {
  ...
  const videoUrl = toAbsoluteUrl(hero.main_body_video.video_separ.url);
...
export const submitContactForm = async (data: ContactFormData): Promise<{ success: boolean; error?: string }> => {
  try {
    const response = await fetch(`${STRAPI}/api/contact-formulars`, {
      method: "POST",
